...
const menuItems: MenuItem[] = [
  { label: "Mes annonces", path: "/dashboard/mes-annonces", icon: ClipboardDocumentListIcon },
  { label: "Messagerie", path: "/messages", icon: ChatBubbleOvalLeftEllipsisIcon },
  { label: "Mes Favoris", path: "/dashboard/mes-favoris", icon: BookmarkIcon },
  { label: "Mes recherches sauvegardées", path: "/mes-recherches", icon: MagnifyingGlassIcon },
  { label: "Paramètres du compte", path: "/dashboard/parametres-compte", icon: Cog6ToothIcon },
  { label: "Espace Candidat", path: "/dashboard/espace-candidat", icon: BriefcaseIcon },
  { label: "Récompenses ambassadeurs", path: "/dashboard/recompenses-ambassadeurs", icon: TrophyIcon },
  { label: "Mon Parrainage", path: "/dashboard/mon-parrainage", icon: UsersIcon },
  { label: "Mon profil", path: "/dashboard", icon: UserCircleIcon },
]

export function SidebarParticularMenu() {
  const [isMediaModalOpen, setIsMediaModalOpen] = useState(false)
  const pathname = usePathname()
  const { logout, userId: authUserId } = useAuthStore()
  const { companyData, loading, error, refreshData } = useUserData()
  const userId = companyData?.userid || authUserId || ""
  const { activeStatus, totalCoins } = useAmbassadorStatus(userId)
  const { unreadCount, fetchUnreadCount } = useMessageStore()

  const router = useRouter()

  // Charger uniquement le compteur au montage (plus léger)
  useEffect(() => {
    console.log("[v0] Sidebar - useEffect triggered")
    
    // Toujours utiliser profileId du localStorage car c'est le plus fiable
    const profileId = localStorage.getItem("profileId")
    
    console.log("[v0] Sidebar - authUserId:", authUserId)
    console.log("[v0] Sidebar - profileId from localStorage:", profileId)
    
    if (profileId) {
      console.log("[v0] Sidebar - Calling fetchUnreadCount with userId:", profileId)
      fetchUnreadCount(profileId)
      // Rafraîchir le compteur toutes les 30 secondes
      const interval = setInterval(() => {
        console.log("[v0] Sidebar - Interval: Refreshing unread count")
        fetchUnreadCount(profileId)
      }, 30000)
      
      // Écouter l'événement de mise à jour des données utilisateur
      const handleUserDataUpdate = () => {
        console.log("[v0] Sidebar - userDataUpdated event received, refreshing data...")
        refreshData()
      }
      ...
      return () => {
        ...
        console.log("[v0] Sidebar - Cleaning up interval")
        clearInterval(interval)
      }
    } else {
      console.log("[v0] Sidebar - No profileId in localStorage, cannot fetch unread count")
    }
  }, [fetchUnreadCount])

  // Log pour déboguer le compteur
  useEffect(() => {
    console.log("[v0] Sidebar - unreadCount:", unreadCount)
  }, [unreadCount])
  
  console.log("[v0] Sidebar - userId:", userId)
  console.log("[v0] Sidebar - activeStatus:", activeStatus)
  console.log("[v0] Sidebar - totalCoins:", totalCoins)
  console.log("[v0] Sidebar - border color:", activeStatus?.hexprimarycolor)
  ...
  const handleSaveMedia = async (file: File) => {
    try {
      ...
      formData.append("photoprofilurl", file)
      ...
      <div className="flex flex-col items-center space-y-4 p-6 border-b border-border">
        <div className="relative group cursor-pointer" onClick={openMediaModal}>
          ...
          >
            {companyData?.photoprofilurl ? (
              <Image
                src={API_URL + companyData.photoprofilurl}
                alt="Photo de profil"
                width={120}
                height={120}
        ...
        {companyData && (
          <div className="w-full space-y-3 text-center">
            <div>
              <h3 className="text-lg font-semibold text-foreground">{companyData.pseudo}</h3>
