...
class _CreerOffreEmploiScreenState extends State<CreerOffreEmploiScreen> {
  ...
  void _prefillFromInitialData() {
    ...
    _linkController.text =
        (data['external_link'] ?? data['link'])?.toString() ?? '';
    _companyNameController.text =
        (data['company_name'] ??
                (data['company'] is Map ? data['company']['name'] : null))
            ?.toString() ??
        '';
    _companyWebsiteController.text =
        (data['company_website'] ??
                (data['company'] is Map ? data['company']['website'] : null))
            ?.toString() ??
        '';
    _showCompanyProfile =
        (data['show_company_presentation'] ??
            (data['company'] is Map
                ? data['company']['show_company_profile']
                : null)) ==
        true;
    // Profile description will be restored in _restoreProfileDescription after frame
    final profileDescPlain =
        (data['profile_description'] ??
            ...
            ?.toString() ??
        '';
    final profileDescDelta =
        data['profile_description_delta'] ??
        (data['profile'] is Map ? data['profile']['description_delta'] : null);
    ...
    _nationwide =
        ...
        true;
    _showGoogleMap =
        (data['show_google_location'] ??
            (data['location'] is Map
                ? data['location']['show_google_map']
                : null)) ==
        true;
    _teleworkPossible =
        (data['remote_work'] ?? data['telework_possible']) == true;
    _acceptMessages = data['accept_messages'] == true;

    // Contract type & work time
    final contractRaw = data['contract_type'];
    _selectedContractType = contractRaw is Map
        ? (contractRaw['value'] ?? contractRaw['name'])?.toString()
        : contractRaw?.toString();
    final workTimeRaw = data['work_time'];
    _selectedWorkTime = workTimeRaw is Map
        ? (workTimeRaw['value'] ?? workTimeRaw['name'])?.toString()
    ...
    if (salaryRaw is Map) {
      ...
      if (_selectedSalaryType == 'RANGE') {
        ...
        _salaryExactController.text =
            (salaryRaw['min'] ?? salaryRaw['exact'])?.toString() ?? '';
      }
    } else {
      ...
      if (data['salary_exact'] != null) {
        _salaryExactController.text = data['salary_exact'].toString();
      }
      _selectedSalaryPeriod = data['salary_period']?.toString();
      final paymentType = data['salary_payment_type']?.toString();
      if (paymentType != null) {
        _isSalaryGross =
            paymentType.toLowerCase().contains('brut') || paymentType == 'brut';
      }
    }
    ...
    if (availRaw is Map) {
      ...
      if (availRaw['start_date'] != null) {
        _availableFromDate = DateTime.tryParse(
          availRaw['start_date'].toString(),
        );
      }
      if (availRaw['end_date'] != null) {
        _availableUntilDate = DateTime.tryParse(
          availRaw['end_date'].toString(),
        );
      }
    } else {
      ...
      if (data['available_from'] != null) {
        _availableFromDate = DateTime.tryParse(
          data['available_from'].toString(),
        );
      }
      if (data['available_until'] != null) {
        _availableUntilDate = DateTime.tryParse(
          data['available_until'].toString(),
        );
      }
    }
    ...
    if (advantagesRaw is List) {
      _selectedAdvantages = advantagesRaw
          .map(
            (a) => a is Map
                ? (a['name'] ?? a['value'] ?? a.toString()).toString()
                : a.toString(),
          )
          .toList();
    }
    ...
    final eduRaw =
        ...
        (data['profile'] is Map ? data['profile']['education_level'] : null);
    _selectedEducationLevel = eduRaw is Map
        ? (eduRaw['value'] ?? eduRaw['name'])?.toString()
        : eduRaw?.toString();
    final expRaw =
        data['experience_level'] ??
        (data['profile'] is Map ? data['profile']['experience_level'] : null);
    _selectedExperienceLevel = expRaw is Map
        ? (expRaw['value'] ?? expRaw['name'])?.toString()
        : expRaw?.toString();

    // Category & function - set names, IDs will be resolved after metadata loads
    final categoryRaw = data['category'];
    _selectedCategoryName = categoryRaw is Map
    ...
    _selectedFunctionName = functionRaw is Map
        ? (functionRaw['name'] ?? functionRaw.toString())
        : functionRaw?.toString();
    if (data['function_id'] != null) {
      _selectedFunctionId = int.tryParse(data['function_id'].toString());
    }
  ...
  void _restoreDescription(Map<String, dynamic> data) {
    ...
    debugPrint(
      'EDIT FORM _restoreDescription: descDelta type=${descDelta?.runtimeType}, value=$descDelta',
    );
  ...
  Future<void> _saveFormProgress() async {
    try {
      ...
      final formData = {
        ...
        'profile_description': _profileDescQuillController.document
            .toPlainText()
            .trim(),
        'profile_description_delta': jsonEncode(
          _profileDescQuillController.document.toDelta().toJson(),
        ),
      };
  ...
  Future<void> _restoreFormData() async {
    try {
      ...
      setState(() {
        ...
        } else if ((formData['profile_description'] ?? '')
            .toString()
            .isNotEmpty) {
          final doc = Document();
      ...
      for (final category in mainList) {
        ...
        if (subList != null) {
          ...
          for (final sub in subList) {
            ...
            final functionId = subId is int
                ? subId
                : int.tryParse(subId.toString());
            if (functionId == null) continue;

            functions.add({'id': functionId, 'name': subLabel});
          }
        }

        categories.add({'id': categoryId, 'name': label});
        functionsMap[categoryId] = functions;
      }

      return {'categories': categories, 'functions': functionsMap};
    } on ApiException {
      rethrow;
    } catch (e) {
  ...
  List<String> _parseStringList(dynamic data) {
    ...
    if (data is List) {
      return data.map((e) {
        ...
        if (e is Map) {
          return (e['name'] ?? e['label'] ?? e['value'] ?? e.toString())
              .toString();
        }
        return e.toString();
      }).toList();
  ...
  void _updateFunctionsForCategory(int? categoryId) {
    ...
    setState(() {
      ...
      _functionsForCategory = List<Map<String, dynamic>>.from(
        _categoryFunctionsMap[categoryId] ?? [],
      );
      _selectedFunctionId = null;
      _selectedFunctionName = null;
    });
    ...
    switch (_currentStep) {
      ...
      case 2: // Step 3: Description
        ...
        if (_selectedSalaryType == 'EXACT' &&
            _salaryExactController.text.trim().isEmpty) {
  ...
  Future<void> _pickMedia() async {
    try {
      ...
      if (validFiles.length < files.length) {
        _showSnack(
          'Certains fichiers dépassent 20 MB et ont été ignorés',
          isError: true,
        );
      }
      setState(() {
  ...
  Future<bool> _uploadMediaFilesToJobOffer(String jobOfferId) async {
    ...
    try {
      ...
      final uri = Uri.parse(
        '${ApiConfig.baseUrl}/job-offers/$jobOfferId/media',
      );
  ...
  Future<void> _submitJobOffer() async {
    ...
    try {
      ...
      if (_selectedSalaryType != null &&
          _selectedSalaryType != 'DEPENDING_ON_PROFILE') {
        salary = {
      ...
      final body = {
        ...
        if (_linkController.text.isNotEmpty)
          'external_link': _linkController.text,
        'title': _titleController.text,
        'description': _descriptionQuillController.document
            .toPlainText()
            .trim(),
        ...
        if (_selectedAdvantages.isNotEmpty)
          'advantages': _toApiValues(_selectedAdvantages),
        'telework_possible': _teleworkPossible,
        'location': {
          'country': 'FR',
          if (_selectedLocation?.address != null &&
              _selectedLocation!.address.isNotEmpty)
            'city': _selectedLocation!.address,
          ...
          if (_selectedLocation?.postalCode != null)
        ...
        'profile': {
          ...
          if (_profileDescQuillController.document
              .toPlainText()
              .trim()
              .isNotEmpty)
            'description': _profileDescQuillController.document
                .toPlainText()
                .trim(),
          if (_profileDescQuillController.document
              .toPlainText()
              .trim()
              .isNotEmpty)
            'description_delta': _profileDescQuillController.document
                .toDelta()
                .toJson(),
        },
      ...
      debugPrint(
        'Submitting job offer (${_isEditMode ? 'edit' : 'create'})...',
      );
      final response = _isEditMode
          ? await ApiClient().authenticatedPut(
              '/job-offers/${widget.jobOfferId}',
              body: body,
            )
          : await ApiClient().authenticatedPost('/job-offers', body: body);
      debugPrint(
        'Job offer ${_isEditMode ? 'updated' : 'created'} response: $response',
      );

      var mediaSuccess = true;
      // Upload media after job offer is created/updated.
      if (_selectedMediaFiles.isNotEmpty) {
        final jobOfferId = _isEditMode
            ? widget.jobOfferId
            : (response['data']?['id'] ??
                      response['id'] ??
                      response['job_offer']?['id'])
                  ?.toString();
            ...
            children: [
              ...
              Text(
                mediaSuccess
                    ...
                    : 'L\'offre est enregistrée, mais certaines images n\'ont pas été envoyées.',
  ...
  Widget _buildStep1Informations() {
    final categoryNames = _categoriesFromApi
        ...
        .toList();
    final functionNames = _functionsForCategory
        ...
        _buildFormCard(
          ...
          children: [
            _buildTextField(
              ...
              fieldKey: 'link',
              helperText:
                  'Le lien permettra d\'extraire automatiquement le titre, la description, le salaire, le type de contrat et autres détails de l\'offre pour faciliter la création de votre annonce.',
            ),
          ],
        ),
        const SizedBox(height: 24),
        ...
        _buildFormCard(
          ...
          children: [
            _buildTextField(
              ...
              fieldKey: 'title',
              helperText:
                  'Saisissez un titre clair et précis pour le poste (ex: "Développeur Full Stack Senior").',
            ),
            const SizedBox(height: 12),
            _buildRichTextEditor(
              ...
              helperText:
                  'Décrivez en détail les missions, responsabilités et environnement de travail. Utilisez les outils de mise en forme.',
            ),
            const SizedBox(height: 8),
            Padding(
            ...
            _buildDropdownField(
              ...
              items: _contractTypes.isNotEmpty
                  ? _contractTypes
                  : [
                      'CDI',
                      'CDD',
            ...
            _buildRadioGroup<String>(
              ...
              labelBuilder: (value) {
                switch (value) {
                  case 'RANGE':
                    return 'Tranche salariale';
                  case 'EXACT':
                    return 'Salaire exact';
                  case 'DEPENDING_ON_PROFILE':
                    return 'Salaire selon le profil';
                  default:
                    return value;
                }
              },
            ...
            if (_selectedSalaryType != null &&
              ...
              _buildDropdownField(
                ...
                onChanged: (val) =>
                    setState(() => _isSalaryGross = val == 'Brut'),
                hint: const Text(
                  'Type de salaire',
              ...
              _buildDropdownField(
                ...
                items: _salaryPeriods.isNotEmpty
                    ? _salaryPeriods
                    : ['YEAR', 'MONTH', 'DAY', 'HOUR'],
            ...
            _buildRadioGroup<String>(
              ...
              labelBuilder: (value) =>
                  value == 'IMMEDIATE' ? 'Immédiatement' : 'À partir de',
              onChanged: (value) =>
                  setState(() => _selectedAvailabilityType = value),
            ),
            const SizedBox(height: 4),
            if (_selectedAvailabilityType == 'FROM_DATE') ...[
              _buildDateField(
                ...
                onDateSelected: (date) =>
                    setState(() => _availableFromDate = date),
              ),
              const SizedBox(height: 12),
            ],
            _buildDateField(
              ...
              onDateSelected: (date) =>
                  setState(() => _availableUntilDate = date),
            ),
            const SizedBox(height: 16),
            _buildCheckboxGroup(
              title: "Avantages :",
              options: _advantagesFromApi.isNotEmpty
                  ? _advantagesFromApi
                  : [
                      "TRANSPORT",
                      "MEAL_VOUCHERS",
              ...
              onChanged: (updatedList) =>
                  setState(() => _selectedAdvantages = updatedList),
            ),
            const SizedBox(height: 16),
            _buildCheckOption(
              ...
              child: Row(
                ...
                children: [
                  Checkbox(
                    ...
                    onChanged: (val) =>
                        setState(() => _showCompanyProfile = val ?? false),
        ...
        _buildFormCard(
          ...
          children: [
            _buildDropdownField(
              ...
              items: _educationLevels.isNotEmpty
                  ? _educationLevels
                  : [
                      'NONE',
                      'CAP',
            ...
            _buildDropdownField(
              ...
              items: _experienceLevels.isNotEmpty
                  ? _experienceLevels
                  : [
                      'JUNIOR',
                      '1_3_YEARS',
            ...
            _buildRichTextEditor(
              ...
              helperText:
                  'Décrivez les compétences, qualités et expériences recherchées pour ce poste.',
            ),
            const SizedBox(height: 8),
            Padding(
              padding: const EdgeInsets.only(left: 4),
  ...
  Widget _buildStep5Review() {
    ...
    if (_selectedSalaryType == 'RANGE' &&
      ...
      salaryDisplay =
          '${_salaryMinController.text} - ${_salaryMaxController.text} €';
    } else if (_selectedSalaryType == 'EXACT' &&
        _salaryExactController.text.isNotEmpty) {
    ...
    return Column(
      ...
      children: [
          ...
          rows: [
            ...
            _buildReviewRow(
              ...
              _selectedAvailabilityType == 'IMMEDIATE'
                  ? 'Immédiatement'
                  : _selectedAvailabilityType == 'FROM_DATE' &&
                        _availableFromDate != null
            ...
            _buildReviewRow(
              'Avantages',
              _selectedAdvantages.isEmpty
                  ? '-'
                  : _selectedAdvantages.join(', '),
            ),
            _buildReviewRow(
              'Télétravail possible',
              _teleworkPossible ? 'Oui' : 'Non',
            ),
            ...
            _buildReviewRow(
              'Afficher présentation entreprise',
              _showCompanyProfile ? 'Oui' : 'Non',
            ),
            _buildReviewRow(
        ...
        _buildReviewSection(
          ...
          rows: [
            ...
            _buildReviewRow(
              ...
              _profileDescQuillController.document.toPlainText().trim().isEmpty
                  ? '-'
                  : _profileDescQuillController.document.toPlainText().trim(),
            ),
          ],
        ),
        const SizedBox(height: 16),
        ...
        _buildReviewSection(
          ...
          rows: [
            _buildReviewRow('Fichiers', () {
              final total =
                  _existingMediaUrls.length + _selectedMediaFiles.length;
              if (total == 0) return 'Aucun média sélectionné';
              final parts = <String>[];
              if (_existingMediaUrls.isNotEmpty) {
                parts.add('${_existingMediaUrls.length} existant(s)');
              }
              if (_selectedMediaFiles.isNotEmpty) {
                parts.add('${_selectedMediaFiles.length} nouveau(x)');
              }
